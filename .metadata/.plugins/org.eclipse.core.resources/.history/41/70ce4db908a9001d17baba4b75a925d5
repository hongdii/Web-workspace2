...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginUser == null) { // 로그인 실패 => 에러페이지 응답
			...
			// 로그인한 회원의 정보를 로그아웃하기 전까지 계속 가져다가 쓸것이기 때문에 
			// session에 데이터 담기
			...
			// 1. 포워딩방식 응답 뷰 출력하기
			// 	  선택된 jsp가 보여질 뿐 url에는 여전히 이 서블릿에 대한 매핑값이 남아있을 것
			//		localhost:8082/jspproject/login.me
			//		url의 내용이 그대로 남아있는 상태에서 새로고침(f5) 누르게되면 계속 로그인 요청을 보내게 될것.
			
			// 2. url재요청방식(sendRedirect 방식)
			//		localhost:8082/jspproject라는 url로 재요청 보냄
			response.sendRedirect(request.getContextPath()); // 로그인시에는 redirect 방식이 사용됨
