...
//		String title = request.getParameter("title"); // null
		...
		/*
		 * 폼 전송시 일반 방식이 아니라 multipart/form-data로 전송하는 경우
		 * request로부터 값을 뽑아낼수 없다.
		 * 
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * 1mbyte -> 1024kbyte -> 1024 * 1024 byte -> (2의 10승)
			 */
			 ...
			 * 다만 WebContents폴더로부터 board_upfiles폴더까지의 경로는 매개변수로 제시해줘야한다.
			 ...
			 * 그리고 사용자가 올린 파일명은 그대로 해당폴더에 업로드 하지 않는게 관례
			 * - 같은 파일명이 있을경우 덮어씌워지거나 에러가 날 수 있고, 한글/특수문자/띄어쓰기가 포함된 파일명일 경우 사용하는
			 * 서버에 따라 에러가 발생할수 있기 때문. 따라서 파일명은 반드시 변환시켜줘서 업로드할 예정
			 * 
			 * 기본적으로 파일명 수정 작업을 해주는 객체
			 *  - DefaultFileNamePolicy 객체(cos.jar에서 제공)
			 *  - 내부적으로 rename()이라는 메소드가 실행이되면서 파일명이 수정됨.
			 *  - 기본적으로 동일한 파일명이 존재할 경우 파일명 뒤에 카운팅된 숫자를 붙여서 유니크한 파일명을 만듬
			 *  	ex) aaa.jpg, aaa1.jpg, aaa2.jpg ...
			 *  
			 *  - 절대 파일명이 안겹치게끔 rename해볼예정
			 *  - 즉, DefaultFilenamePolicy객체는 사용하지 않음
			 *  내 입맛대로 나만의 파일명 생성 규칙을 만들어서 파일이름을 바꿔주는 객체를 만들어보기.
			 */
			MultipartRequest multi = new MultipartRequest(request, savePath, maxSize, "UTF-8", 
					new MyFileRenamePolicy());
			// 3. DB에 기록할 데이터들을 뽑아서 Attachment객체에 담기, Board객체에 각각 담아주기
			// - 카테고리번호, 제목, 내용, 작성자번호를 뽑아서 Board에 Insert
			// - 넘어온 첨부파일이 있다면, 원본명, 수정명, 폴더경로를 뽑아서 Attachment테이블에 Insert
			...
			Attachment at = null; // 처음에는 null값으로 초기화시키고, 실제로 사용자가 첨부파일을 업로드 했을때만 객체 생성
			
			// 첨부파일이 있을경우 원본명을 반환해주고, 없을경우 null값을 반환해줌
			if(multi.getOriginalFileName("upfile") != null) {
				...
				at.setFilePath("resources/board_upfiles/"); // 앞에 슬레시 추가유무는 자유임
			}
			...
			if(result > 0) { // 성공적으로 작성 => 최신글 목록으로 이동
				...
				if(at != null) {
					// 삭제시키고자하는 파일객체 생성후 delete메소드 호출시 파일이 삭제된다.
