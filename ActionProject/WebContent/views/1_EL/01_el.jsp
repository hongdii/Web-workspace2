...
<body>
	<h3>1. 기존방식대로 스클립틀릿과 표현식을 이용해서 각 영역에 담겨있는 값을 뽑아서 화면에 출력</h3>
	<%
	...
	<p>
		...
		내부적으로 해당 scope영역에 해당 키값의 밸류값을 가져올수있다.<br>
		기본적으로 EL은 JSP내장객체를 구분하지 않고 자동으로 모든 내장객체에 키값을 검색해서 존재하는 경우 해당값을 가져옴
	</p>
	
	<p>
<!-- 		request영역안에 담겨있는 classRoom 가져옴 -->
<!-- 		잘못된값 입력시 null값 출력이 아니라 아무값도 출력되지않음 
			변수명만 작성해도 JSP에 내장된 scope영역에서 가져오기땜에 값가져오기가능
			-->
		학원명 : ${academy } <br>	
		강의장 : ${classRoom } <br>
		강사정보 : ${teacher.name }, ${teacher.gender }, ${teacher.job } <br>
		<!-- teacher라는 키값에 접근했을때 밸류값이 Person객체임
			해당 Person객체의 각 필드에 담긴값들을 출력하고자한다면 키값.필드명으로 접근하면됨 
			★ 내부적으로 해당 필드의 getter메소드를 찾아서 실행하는 구조(getter메소드가 없다면 실행이안됨)	
				필드 자체를 가져오는게아니라 getName()을 호출하는거임
			-->
		<ul>
		<!-- EL표현식으로 가급적이면 사용 -->
	...
	<!-- 
		...
		page -> request -> session -> application 순으로 속성을 찾아나간다
		현재 찾고있는 scope범위 안에서 내가 찾고자하는 속성이 존재한다면 , 반복을 멈추고 해당속성의 속성값을 출력함.
		
		만약에 모든 영역에서 못찾을 경우 ? 빈화면이 출력됨. 오류는 나지않음		
	 -->
	 ...
	 applicationScope에 담긴값 : ${applicationScope.scope }<br><br>
