...
=> 즉, 해당 scope영역에 setAttribute라는 메소드를 이용해서 key + value 형태로 데이터를 담아놓는것과 동일한 형태다
=> c:set을 통해 선언된 변수는 EL로 접근해서 사용 가능

* 주의사항
- 변수의 타입을 별도로 지정하지 않음(Object 자료형이 들어간다고 생각)
...
<!-- 
	변수명만 제시하면 공유범위가 가장 작은곳에서부터 큰곳으로 순차적으로 찾아지게됨. 티가 나지는 않지만 속도가 조금 느려질수가 있다.
...
request에 있는 result변수 삭제 후 result : ${result } <br>
...
<pre>
* 변수(데이터) 출력 : (&lt;c:out value="출력하고자 하는 값" default="기본값(생략가능)" escapeXml="true/fasle(기본값:true, 생략가능)") &gt;)
...
<!-- escapeXml -->
<c:set var="outTest" value="<b>출력테스트</b>" /> <br> <!-- escapeXml 생략시 기본값은 true == 태그 해석안됨(문자열로 취급함) -->
...
${ num1 gt num2 } 은 문자열과 문자열로 비교하는 것이기때문에 num2가 더 커서 false값 나옴
<br>

<!-- EL표현식 안에는 자료형 개념이 없어서 형변환 못함 -->
...
<c:if test="${ str eq '안녕' }">
	<mark>Hello Word!</mark>
</c:if>

<c:if test="${str ne '안녕하세여' }">
	<mark>안녕히가세요</mark>
</c:if>
...
<c:choose>
	...
	<c:otherwise>
		<b>num1은 10 혹은 2와 일치하지 않습니다</b> <br>
	</c:otherwise>
</c:choose>
...
<%-- 					<td>${list[status.index].name }</td> <!-- status.count와 동일 --> --%>
					<td>${p.name }</td>
					<td>${p.gender }</td>
					<td>${p.job }</td>
				</tr>
