...
<!-- 
		...
		단, 시작태그와 종료태그 사이에 값이 존재할 필요가 없는 경우 시작태그에 /로 닫아줘야함
	 -->
	 <%-- 	 <%=year %> 사용못함 
			include.jsp실행후에 footer.jsp 실행
			장점 : 파일이가벼움
			단점 : 공유어려움 
			
			include.jsp footer.jsp는 각각 다른 영역에 위치함
			따라서 같은 변수명 사용해도 문제없음 
			jsp:include 로 하는 작업 권장 --%>
	 
	 <br><br>
	 
	 특징 1: include하고있는 페이지에 선언된 변수를 공유하지않는다.(즉, 동일한 이름의 변수 선언 가능)<br>
	 <% String year="2023"; %>
	 
	 특징 2: 포함시 include하는 페이지로 내가 원하는 값을 전달할수도 있다<br>
	 <!-- 삼항연산자 사용 가능 -->
	 <jsp:include page="footer.jsp">
